...
{
    public class LtcBlockApiTests : IClassFixture<CustomWebApplicationFactory<Program>>
    {
        private readonly HttpClient _client;
        public LtcBlockApiTests(CustomWebApplicationFactory<Program> factory)
        {
        ...
        [Fact]
        public async Task Get_LtcBlocks_Returns_OK()
        {
            var content = new StringContent("{}", Encoding.UTF8, "application/json");
            var response = await _client.PostAsync("/api/LtcBlock/fetch", content);
        ...
        [Fact]
        public async Task Fetch_Save_LtcBlocks_Returns_OK()
        {
            // Arrange
            var mockService = new Mock<ILtcBlockService>();
            var mockLogger = new Mock<ILogger<LtcBlockController>>();

            var fakeBlock = new LtcBlock { Height = 123456 };

            mockService.Setup(s => s.FetchAndSaveAsync())
                       .ReturnsAsync(fakeBlock);

            var controller = new LtcBlockController(mockService.Object, mockLogger.Object);
