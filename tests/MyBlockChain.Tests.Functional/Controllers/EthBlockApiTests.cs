...
namespace MyBlockChain.Tests.Functional.Controllers
{
    public class EthBlockApiTests : IClassFixture<CustomWebApplicationFactory<Program>>
    {
        ...
        {
            // Arrange
            var mockService = new Mock<IEthBlockService>();
            var mockLogger = new Mock<ILogger<EthBlockController>>();

            var fakeBlock = new EthBlock { Height = 123456 };

            mockService.Setup(s => s.FetchAndSaveAsync())
                       .ReturnsAsync(fakeBlock);

            var controller = new EthBlockController(mockService.Object, mockLogger.Object);
